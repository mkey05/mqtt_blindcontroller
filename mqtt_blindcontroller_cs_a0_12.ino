...
enum LedColor {
  ...
  LED_LIMIT_UPPER,  // Purple - Waiting to save new OPEN limit
  LED_LIMIT_LOWER,  // Orange - Waiting to save new CLOSED limit
...
#define MANUAL_LIMIT_WINDOW 1500UL
...
// After a manual UP/DOWN movement, the next quick press
// of the SAME button saves that position as the limit.
enum ManualLimitPending {
  LIMIT_NONE,
  LIMIT_UPPER,
  LIMIT_LOWER
};

ManualLimitPending manualLimitPending = LIMIT_NONE;

unsigned long manualLimitPendingTime = 0;

// Prevent the button press used to STOP automatic movement
// from subsequently being interpreted as a new command.
bool stopButtonConsumed = false;

// Prevent a limit-save press from becoming a long press
// if the user happens to hold it.
bool limitSaveTriggered = false;
...
void flash_LED(int noTimes, LedColor color) {
  ...
  // Do not override the manual-limit indication.
  // The pending limit colour must remain visible
  // until the 1.5 second window expires.
  if (manualLimitPending != LIMIT_NONE) {

    updateManualLimitLED();
...
void setLedColor(LedColor color) {
  ...
  switch (color) {
    ...
    case LED_LIMIT_UPPER:
      // Purple = waiting to save new OPEN / 100% limit
      pixels.setPixelColor(0, 165, 0, 255);
      break;

    case LED_LIMIT_LOWER:
      // Orange = waiting to save new CLOSED / 0% limit
...
void updateManualLimitLED() {

  // -------------------------------------------------
  // Pending OPEN / 100% limit
  // -------------------------------------------------

  if (manualLimitPending == LIMIT_UPPER) {

    // Pending-limit indication has priority over
    // normal flashing/status LEDs.
    ledActive = false;
    ledOn = true;

    setLedColor(LED_LIMIT_UPPER);

    return;
  }

  // -------------------------------------------------
  // Pending CLOSED / 0% limit
  // -------------------------------------------------

  if (manualLimitPending == LIMIT_LOWER) {
...
void updateLED() {
  ...
  // =================================================
  // MANUAL LIMIT PENDING HAS HIGHEST PRIORITY
  // =================================================

  if (manualLimitPending != LIMIT_NONE) {

    // Keep the pending-limit colour solid.
    ledActive = false;
    ledOn = true;
    ...
    else if (manualLimitPending == LIMIT_LOWER)
      setLedColor(LED_LIMIT_LOWER);
...
void serviceMotion() {
  ...
  int finalCreepDistance = max(1, travelLength / 80);
...
void handleTargetMotion() {
  ...
  // Manual button movement is handled entirely
  // by upButtonCheck() / downButtonCheck().
  ...
  if (stopPressed) {
    ...
    if (stopPressTime == 0) {
      ...
      // The physical button press has been consumed as STOP.
      // Do not let it subsequently become a new UP/DOWN command.
      stopButtonConsumed = true;
...
// ============================================================
// MANUAL MOTION SERVICE
// ============================================================

void serviceManualMotion() {
  ...
  // Smooth acceleration during manual movement
  acceleration += 10;
  ...
  // Publish actual position while manually moving
  if (
    motionState != MOTION_IDLE && millis() - lastMQTTPosition >= MQTT_POSITION_INTERVAL) {
...
void setManualUpperLimit() {
  ...
  Serial.print("🔵 Manual UPPER limit set: ");
  Serial.println(openPosition);

  sendMQTTMessage();
}

void setManualLowerLimit() {
  ...
  Serial.print("🟡 Manual LOWER limit set: ");
...
void downButtonCheck() {
  ...
  // -------------------------------------------------
  // EXPIRE MANUAL LIMIT WINDOW
  // -------------------------------------------------

  if (
    manualLimitPending == LIMIT_LOWER && millis() - manualLimitPendingTime > MANUAL_LIMIT_WINDOW) {

    manualLimitPending = LIMIT_NONE;
    limitSaveTriggered = false;

    setLedColor(LED_OFF);

    Serial.println("⌛ LOWER limit save window expired");
  }
  ...
  // Ignore DOWN if UP is also pressed
  ...
  if (downPressed) {
    ...
    if (!downButtonActive) {
      ...
      // -------------------------------------------------
      // AUTOMATIC STOP PRESS
      // -------------------------------------------------

      if (stopButtonConsumed) {

        Serial.println("⏹ DOWN press already consumed as STOP");

        return;
      }

      // -------------------------------------------------
      // QUICK PRESS AFTER MANUAL DOWN
      // Save current position as LOWER / 0% limit.
      // -------------------------------------------------

      if (
        manualLimitPending == LIMIT_LOWER && millis() - manualLimitPendingTime <= MANUAL_LIMIT_WINDOW) {

        Serial.println("🟡 Saving current position as LOWER limit");

        limitSaveTriggered = true;
        manualLimitPending = LIMIT_NONE;
      ...
      // A different button cancels the pending limit action.
      if (manualLimitPending != LIMIT_NONE) {
        manualLimitPending = LIMIT_NONE;
      }
    }
    ...
    if (
      !downLongPressActive && !limitSaveTriggered && millis() - downPressStart >= LONGPRESSTIME) {

      downLongPressActive = true;

      // Long press is manual movement.
      // It does NOT change the target to 0%.
      ...
      // Immediately tell HomeKit we're CLOSING.
      sendMQTTMessage();
  ...
  if (downButtonActive && digitalRead(DOWNBUTTONPIN) == LOW) {

    downButtonActive = false;

    // If this button was used to stop automatic movement,
    // do NOT treat the release as another command.
    if (stopButtonConsumed) {
      stopButtonConsumed = false;
      downLongPressActive = false;
      downPressStart = 0;
      Serial.println("DOWN release consumed after automatic stop");
      return;
    }
    ...
    if (downLongPressActive) {
      ...
      travelPercent = constrain(travelPercent, 0, 100);
      ...
      // Start the window for saving a new CLOSED limit
      manualLimitPending = LIMIT_LOWER;
      manualLimitPendingTime = millis();

      updateManualLimitLED();

      Serial.println("⏹ Manual DOWN released");
      Serial.println("🟡 Quick DOWN press will save LOWER limit");

      // VERY IMPORTANT:
      // Never allow this release to become a short press.
      downPressStart = 0;
      return;
    }
    ...
    // If we get here, it really was a short press.
    Serial.println("⬇️ Short DOWN press -> CLOSE");
    switchPosition = 0;
    target();
    return;
  }
...
void upButtonCheck() {
  ...
  // -------------------------------------------------
  // EXPIRE UPPER LIMIT WINDOW
  // -------------------------------------------------

  if (
    manualLimitPending == LIMIT_UPPER && millis() - manualLimitPendingTime > MANUAL_LIMIT_WINDOW) {
    manualLimitPending = LIMIT_NONE;
    limitSaveTriggered = false;
    setLedColor(LED_OFF);
    Serial.println("⌛ UPPER limit save window expired");
  }
  ...
  // Ignore UP if DOWN is also pressed
  ...
  if (upPressed) {
    ...
    if (!upButtonActive) {
      ...
      if (stopButtonConsumed) {
        ...
        Serial.println("⏹ UP press already consumed as STOP");

        return;
      }

      // -------------------------------------------------
      // QUICK PRESS AFTER MANUAL UP
      // Save current position as UPPER / 100% limit.
      // -------------------------------------------------

      if (
        manualLimitPending == LIMIT_UPPER && millis() - manualLimitPendingTime <= MANUAL_LIMIT_WINDOW) {

        Serial.println("🔵 Saving current position as UPPER limit");
    ...
    if (
      !upLongPressActive && !limitSaveTriggered && millis() - upPressStart >= LONGPRESSTIME) {

      upLongPressActive = true;

      // Long press is manual movement.
      // It does NOT change the target to 100%.
      ...
      // Immediately tell HomeKit we're OPENING.
      sendMQTTMessage();
  ...
  if (upButtonActive && digitalRead(UPBUTTONPIN) == LOW) {
    ...
    if (stopButtonConsumed) {
      ...
      upPressStart = 0;
      Serial.println("UP release consumed after automatic stop");
      return;
    }
    ...
    if (upLongPressActive) {
      ...
      // Start the window for saving a new OPEN limit
      manualLimitPending = LIMIT_UPPER;
      manualLimitPendingTime = millis();

      updateManualLimitLED();

      Serial.println("⏹ Manual UP released");
      Serial.println("🔵 Quick UP press will save UPPER limit");
    ...
    // If we get here, it really was a short press.
    Serial.println("⬆️ Short UP press -> OPEN");
    switchPosition = 100;
    target();
